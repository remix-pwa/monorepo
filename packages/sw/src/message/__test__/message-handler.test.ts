...
describe('MessageHandler Testing Suite', () => {
  ...
  test('handleMessage catches and logs errors from event handlers successfully', async () => {
    ...
    const spiedOnConsole = vi.spyOn(console, 'error');
    ...
    expect(mockHandler).toHaveBeenCalledTimes(1);
    expect(spiedOnConsole).toHaveBeenCalledTimes(1);
    expect(spiedOnConsole).toHaveBeenCalledWith('Error handling message of type test:', new Error('Test Error'));

    mockHandler.mockClear();
    spiedOnConsole.mockRestore();
  });
});
