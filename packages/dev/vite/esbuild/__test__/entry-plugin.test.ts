...
import type { ResolvedEsbuildConfig } from '../../types.js';
import entryModulePlugin from '../entry-plugin.js';

describe('entry-module plugin', () => {
  const config = {
    appDirectory: '/path/to/app',
    entryWorkerFile: 'entry-worker.js',
    serviceWorkerPath: '/path/to/app/entry-worker.js',
    routes: {
  ...
  test('should inject the routes and entry worker into the entry module', async () => {
    ...
    expect(result.contents).toMatchInlineSnapshot(`
      "
            import * as entryWorker from \\"/path/to/app/entry-worker.js\\";
