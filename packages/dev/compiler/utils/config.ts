...
export default async function readConfig(remixRoot: string, mode: ServerMode): Promise<ResolvedWorkerConfig> {
  ...
  console.log(remixConfig, 'REMIX CONFIG');
  console.log(findConfig(remixRoot, 'remix.config', EXTENSIONS), 'FIND CONFIG');
  const workerConfig = (await import(
    pathToFileURL(findConfig(remixRoot, 'remix.config', EXTENSIONS) as string).href
  ).then(m => m.default ?? m)) as WorkerConfig;
