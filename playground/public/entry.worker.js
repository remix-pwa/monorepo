...
function createRouter(init) {
  ...
  function getScrollKey(location2, matches) {
    if (getScrollRestorationKey) {
      let key = getScrollRestorationKey(location2, matches.map((m) => createUseMatchesMatch(m, state.loaderData)));
